...
public class CrabTrapArmInteractionPoint extends ArmInteractionPoint {
    ...
    public ItemStack insert(ItemStack stack, boolean simulate) {
        if (level.getBlockEntity(pos) instanceof CrabTrapBlockEntity interaction) {
            ...
            if (cap.isPresent())
                return cap.map(handler -> handler.insertItem(0, stack, simulate)).orElse(stack);
        }
        return stack;
    }
    ...
    public ItemStack extract(int slot, int amount, boolean simulate) {
        if (level.getBlockEntity(pos) instanceof CrabTrapBlockEntity interaction) {
            ...
            if (cap.isPresent())
                return cap.map(handler -> handler.extractItem(slot, amount, simulate)).orElse(ItemStack.EMPTY);
        }
    ...
    public int getSlotCount() {
        if (level.getBlockEntity(pos) instanceof CrabTrapBlockEntity interaction) {
            var cap = interaction.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.DOWN);
            if (cap.isPresent())
                return cap.map(handler -> handler.getSlots()).orElse(0);
        }
        return 0;
    }
